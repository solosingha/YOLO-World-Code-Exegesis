...
class RepMaxSigmoidCSPLayerWithTwoConv(CSPLayerWithTwoConv):
    ...
    def forward(self, x: Tensor, guide: Tensor) -> Tensor:
        """Forward process."""
        x_main = self.main_conv(x)  # 对输入 x 进行卷积，得到 x_main
        x_main = list(x_main.split((self.mid_channels, self.mid_channels), 1)) #在第一个维度上分割，每个维度大小是mid_channels。语法有误
        x_main.extend(blocks(x_main[-1]) for blocks in self.blocks)  # 对 x_main 的最后一部分应用多个 block
        x_main.append(self.attn_block(x_main[-1], guide)) # 对 x_main 的最后一部分应用注意力机制
        return self.final_conv(torch.cat(x_main, 1)) # 将 x_main 的所有部分按通道拼接，然后通过最终的卷积
