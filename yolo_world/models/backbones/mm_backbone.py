...
class HuggingCLIPLanguageBackbone(BaseModule):
    ...
    # 是一个模块化的骨干网络类，通过 @ MODELS.register_module()
    # 注册到某个模型框架中。
    def __init__(self,
                 model_name: str,#指定所使用的预训练模型名称（如 "openai/clip-vit-base-patch32"）。
                 frozen_modules: Sequence[str] = (),#指定需要冻结的模型部分（如 ["all"] 冻结全部模块）
                 dropout: float = 0.0,
                 training_use_cache: bool = False,#是否在训练期间使用缓存。
                 init_cfg: OptMultiConfig = None) -> None:#用于初始化配置的可选参数

        super().__init__(init_cfg=init_cfg)#调用父类的初始化方法 super().__init__(init_cfg=init_cfg)。

        self.frozen_modules = frozen_modules#设置类属性
        self.training_use_cache = training_use_cache#设置类属性
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)#分词器加载： 使用 AutoTokenizer 从 Hugging Face 加载与指定模型名称对应的分词器。
        clip_config = CLIPTextConfig.from_pretrained(model_name,
                                                     attention_dropout=dropout)#使用 CLIPTextConfig 配置 CLIP 文本模型的 dropout。
        self.model = CLIPTP.from_pretrained(model_name, config=clip_config)
        self._freeze_modules()#调用 _freeze_modules 方法，根据 frozen_modules 参数冻结指定部分或全部模型。

    # 文本分词逻辑
    def forward_tokenizer(self, texts):
        if not hasattr(self, 'text'):#如果没有缓存的 self.text，则分词并将结果存入 self.text，以避免重复计算。
            text = list(itertools.chain(*texts)) #使用 itertools.chain 将嵌套的列表展平成一维列表。
            text = self.tokenizer(text=text, return_tensors='pt', padding=True) #使用 tokenizer 对文本进行分词，返回 PyTorch 张量格式（return_tensors='pt'）
            self.text = text.to(device=self.model.device)#张量移动到模型所在设备（GPU/CPU）
        return self.text

    def forward(self, text: List[List[str]]) -> Tensor:
        num_per_batch = [len(t) for t in text]
        assert max(num_per_batch) == min(num_per_batch), (
            'number of sequences not equal in batch')  #检查输入文本每个 batch 的长度是否一致。
        text = list(itertools.chain(*text)) #将嵌套的列表展平并进行分词。
        text = self.tokenizer(text=text, return_tensors='pt', padding=True)
        text = text.to(device=self.model.device)
        txt_outputs = self.model(**text)#将分词后的数据输入 CLIP 模型，获取输出的文本嵌入 txt_outputs.text_embeds。
        txt_feats = txt_outputs.text_embeds
        txt_feats = txt_feats / txt_feats.norm(p=2, dim=-1, keepdim=True)#对嵌入向量进行归一化（L2 范数）。
    ...
    # 如果frozen_modules为空，则不执行冻结。
    # 如果frozen_modules 包含"all"，则冻结模型中的所有参数。
    # 否则，仅冻结名称以指定前缀开头的模块。
