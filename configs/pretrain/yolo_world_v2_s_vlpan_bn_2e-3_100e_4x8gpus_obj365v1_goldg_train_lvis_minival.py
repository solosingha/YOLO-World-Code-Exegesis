...
custom_imports = dict(imports=['yolo_world'],
                      allow_failed_imports=False)#定义基础配置文件的路径和自定义导入模块
...
text_channels = 512
neck_embed_channels = [128, 256, _base_.last_stage_out_channels // 2]#pinjinceng
neck_num_heads = [4, 8, _base_.last_stage_out_channels // 2 // 32]
# base_lr = 2e-3
base_lr = 1e-3  # 原值是2e-3，根据batch size调整
weight_decay = 0.05 / 2
...
# dataset settings
text_transform = [#数据集设置和管道设置，。它还定义了用于训练的数据加载器，包含多个数据集的组合。
...
# training settings，训练设置，包含训练过程中的钩子，训练配置和优化器设置
